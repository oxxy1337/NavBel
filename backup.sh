...
echo "docker exec  navbel_api_1 bash /var/backups/backup.sh ; mv -f $DIR/backups/files/* /var/www/html/backups/files/ ;  mv -f $DIR/backups/db/* /var/www/html/backups/db/ ; chown www-data:www-data /var/www/html/backups ; docker exec  navbel_api_1 php /var/www/html/cron.php" >>  backups/init.sh; 
